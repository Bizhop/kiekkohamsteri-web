...
import { IconButton, Tooltip, Button, TableContainer, Paper, Table, TableRow, TableBody, TableCell, TableHead } from "@mui/material"
            ...
            <div>
                <TableContainer component={Paper}>
                    <Table size="small">
                        <TableHead>
                            <TableCell colSpan={3}>
                                <strong>Käyttäjät ({props.group.name}, {length(props.users)})</strong>
                            </TableCell>
                        </TableHead>
                        <TableBody>
                            {props.users.map(user => {
                                ...
                                return (
                                    <TableRow key={user.id}>
                                        <TableCell>{isGroupAdmin(userGroup) && <Tooltip title="Ylläpitäjä"><EngineeringIcon /></Tooltip>}
                                            <NavLink
                                                to={`/users/${user.id}`}
                                                target="_user"
                                            >
